...
bool Texture2DArray::upload(const gli::texture2DArray &texture, bool gm) {
	if (size_type({ texture.dimensions().x, texture.dimensions().y, texture.layers() }) != this->size) {
...
bool Texture2DArray::upload_layer(int layer, const gli::texture2D &texture) {
	...
	if (texture.format() != this->format || texture.dimensions() != decltype(texture.dimensions())({ this->size.x, this->size.y })) {
		ste_log_error() << "Texture format and size can not be changed!";
		assert(false);
		return false;
	}
