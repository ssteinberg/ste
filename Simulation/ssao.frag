...
uniform float min_step_size = 3;
uniform float max_step_size = 10;
...
vec3 unproject(vec2 frag_coords, float depth) {
	...
	vec4 t = proj_inv * vec4(p, depth*2.0f - 1.0f, 1);
	return t.xyz / t.w;
}
...
void main() {
	...
	vec3 offset = position + normal * .2f;
	...
	for (int i=0; i<points; ++i) {
		...
		for (int j=1; j<steps; ++j, s*=1.5f) {
			...
			for (int k=0; k<3; ++k) {
				...
				//float layer_d = texelFetch(depth_layer, ivec3(ivec2(p / pow(2,j)), k), j).x;
				if (layer_d==1.0f)
					break;
			...
			float l = length(v);
			v /= l;
			float d = max(0, dot(v,normal)) / pow(l,1.4);
