...
attributed_string_common<char> attrib::operator()(std::string &&str) const {
	attributed_string_common<char> newstr(std::move(str));
	newstr.add_attrib({ 0,static_cast<std::uint32_t>(newstr.length()) }, *this);
	return newstr;
}

attributed_string_common<char16_t> attrib::operator()(std::u16string &&str) const {
	attributed_string_common<char16_t> newstr(std::move(str));
	newstr.add_attrib({ 0,static_cast<std::uint32_t>(newstr.length()) }, *this);
	return newstr;
}

attributed_string_common<char32_t> attrib::operator()(std::u32string &&str) const {
	attributed_string_common<char32_t> newstr(std::move(str));
	newstr.add_attrib({ 0,static_cast<std::uint32_t>(newstr.length()) }, *this);
	return newstr;
}

attributed_string_common<wchar_t> attrib::operator()(std::wstring &&str) const {
