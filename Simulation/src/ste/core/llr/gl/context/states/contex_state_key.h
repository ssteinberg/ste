...
private:
	...
	std::int32_t index0{ 0 }, index1{ 0 };

public:
	context_state_key(context_state_name name): name(name) {}
	context_state_key(context_state_name name, std::int32_t index): name(name), index0(index) {}
	context_state_key(context_state_name name, std::int32_t index0, std::int32_t index1): name(name), index0(index0), index1(index1) {}
...
template <> struct hash<StE::LLR::context_state_key> {
	size_t inline operator()(const StE::LLR::context_state_key &x) const {
		...
		return StE::hash_combine(std::hash<std::int64_t>()(h1), std::hash<std::int32_t>()(x.name));
	}
};
