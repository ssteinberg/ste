...
class texture_2d_array : public texture_mipmapped<core_resource_type::Texture2DArray> {
private:
	using Base = texture_mipmapped<core_resource_type::Texture2DArray>;

public:
	texture_2d_array(texture_2d_array &&m) = default;
	texture_2d_array& operator=(texture_2d_array &&m) = default;

	texture_2d_array(gli::format format, const typename Base::size_type &size, int levels = 1) : texture_mipmapped(format, size, levels) {}
	texture_2d_array(const gli::texture2d_array &t, bool generate_mipmaps = false)
