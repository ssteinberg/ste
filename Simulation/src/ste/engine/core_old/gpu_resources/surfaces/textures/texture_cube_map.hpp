...
class texture_cube_map : public texture_mipmapped<core_resource_type::TextureCubeMap> {
private:
...
public:
	...
	image<T> operator[](CubeMapFace face) const {
	...
	image<T> make_image(CubeMapFace face, int level) const {
		return image<T>(*this,
						get_image_container_size(),
						format, image_access_mode::ReadWrite,
