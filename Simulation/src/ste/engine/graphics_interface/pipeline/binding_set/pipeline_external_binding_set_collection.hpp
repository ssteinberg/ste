...
private:
	...
	using set_layouts_t = std::vector<pipeline_external_binding_set_layout>;
	
	using collection_t = boost::container::flat_map<pipeline_layout_set_index, pipeline_external_binding_set>;
	using name_binding_map_t = boost::container::flat_map<std::string, const pipeline_external_binding_layout*>;
...
public:
		...
		pool(pool)
	{
		...
		std::vector<const layout_t*> layout_ptrs;
