...
private:
	auto allocate_pool_instance(const std::vector<const pipeline_binding_set_layout*> &layouts) const {
		auto sets_count = layouts.size();
		std::vector<const pipeline_binding_set_layout_binding*> pool_bindings;
		pool_bindings.reserve(sets_count * 10);

		// Copy bindings
		for (auto &l : layouts)
			pool_bindings.insert(pool_bindings.end(), l->get_bindings().begin(), l->get_bindings().end());
...
public:
	...
	*/
	auto allocate_binding_sets(const std::vector<const pipeline_binding_set_layout*> &layouts) {
		// Create a key
		auto key = key_counter.fetch_add(1);

		// Create new slot
		auto instance = allocate_pool_instance(layouts);
		instance->release_func = binding_set_pool_instance::release_func_t([this, key]() {
			this->release_one(key);
		});

		// Allocate
		auto sets = instance->allocate(layouts);
