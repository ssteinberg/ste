...
public:
	...
	sphere(unsigned vert_slices, unsigned horz_slices, float theta_min = -glm::half_pi<float>(), float theta_max = glm::half_pi<float>()) {
		...
		for (unsigned x = 0; x <= horz_slices; ++x) {
			const  float phi = x == horz_slices ? -glm::pi<float>() : glm::mix(-glm::pi<float>(), glm::pi<float>(), static_cast<float>(x) / static_cast<float>(horz_slices - 1));
			for (unsigned y = 0; y < vert_slices; ++y) {
				...
				v.p().y = metre(glm::sin(theta));
				v.p().x = metre(glm::cos(theta) * glm::sin(phi));
				v.p().z = metre(glm::cos(theta) * glm::cos(phi));

				const glm::vec3 n = v.p().v();
				glm::vec3 t;
				...
				v.uv().x = .5f + glm::atan(static_cast<float>(v.p().x), 
										   static_cast<float>(v.p().z)) / glm::two_pi<float>();
		...
		for (unsigned x = 0; x < horz_slices; ++x) {
			for (unsigned y = 0; y < vert_slices - 1; ++y) {
				const std::uint32_t i0 = x * vert_slices + y;
				const std::uint32_t i1 = (x + 1) * vert_slices + y;
				const std::uint32_t i2 = x * vert_slices + y + 1;
				const std::uint32_t i3 = (x + 1) * vert_slices + y + 1;
