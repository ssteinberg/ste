...
void directional_shadowmap_projector::set_context_state() const {
	...
	GL::gl_current_context::get()->disable_state(GL::BasicStateName::CULL_FACE);
//	GL::gl_current_context::get()->cull_face(GL_FRONT);
	...
	scene->get_directional_shadow_projection_buffers().idb.buffer().bind();
	scene->bind_buffers();
	...
	8_storage_idx = scene->get_directional_shadow_projection_buffers().proj_id_to_light_id_translation_table;
