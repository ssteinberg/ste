...
float shadow(samplerCubeArrayShadow shadow_depth_maps, samplerCubeArray shadow_maps, uint light, vec3 shadow_v, float light_radius) {
	...
	// Number of clusters to sample calculated heuristiccally using the penumbra size
 ...
 */
vec3 shadow_occluder(samplerCubeArray shadow_maps, uint light, vec3 shadow_v, float light_radius) {
	const float cutoff = .5f;
	...
	float d = shadow_blocker_search(shadow_maps, light, norm_v, m);
