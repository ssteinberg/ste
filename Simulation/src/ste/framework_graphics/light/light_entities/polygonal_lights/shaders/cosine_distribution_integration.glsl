...
vec3 integrate_cosine_distribution_quad(vec3 P, 
	...
	vec3 v[4];
	v[0] = M * (light_pos + ltc_point(ltc_points[offset + 0]) - P);
	v[1] = M * (light_pos + ltc_point(ltc_points[offset + 1]) - P);
	v[2] = M * (light_pos + ltc_point(ltc_points[offset + 2]) - P);
	v[3] = M * (light_pos + ltc_point(ltc_points[offset + 3]) - P);
...
vec3 integrate_cosine_distribution_polygon(vec3 P, 
	...
	uint t=0;
	vec3 start = M * (light_pos + ltc_point(ltc_points[offset]) - P);
	vec3 p0,p1,p2;
	if (start.z < .0f) {
		vec3 next = M * (light_pos + ltc_point(ltc_points[offset + 1]) - P);
		for (t=0; t<points-1 && next.z<.0f; ++t) {
			start = next;
			next = M * (light_pos + ltc_point(ltc_points[offset + (t + 2) % points]) - P);
		}
		if (next.z < .0f)
			return vec3(0);
		...
		p1 = M * (light_pos + ltc_point(ltc_points[offset + 1]) - P);
	
	start = normalize(start);

	p0 = start;
	p1 = normalize(p1);
	p2 = normalize(M * (light_pos + ltc_point(ltc_points[offset + (t + 2) % points]) - P));
	...
	for (; t<points; ++t) {
		...
		p0 = p1;
		p1 = p2;
		p2 = next_idx < points ? normalize(M * (light_pos + ltc_point(ltc_points[offset + next_idx]) - P)) : start;
	}
...
vec3 integrate_cosine_distribution_convex_polyhedron(vec3 P, 
	...
	for (int t=0; t<primitives; ++t) {
		vec3 v[3];
		v[0] = M * (light_pos + ltc_point(ltc_points[offset + 3*t + 0]) - P);
		v[1] = M * (light_pos + ltc_point(ltc_points[offset + 3*t + 1]) - P);
		v[2] = M * (light_pos + ltc_point(ltc_points[offset + 3*t + 2]) - P);
