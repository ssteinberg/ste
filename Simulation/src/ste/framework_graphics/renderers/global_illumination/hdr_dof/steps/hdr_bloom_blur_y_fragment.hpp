...
class hdr_bloom_blur_y_fragment : public gl::fragment_graphics<hdr_bloom_blur_y_fragment> {
	using Base = gl::fragment_graphics<hdr_bloom_blur_y_fragment>;

	gl::task<gl::cmd_draw> draw_task;

public:
	hdr_bloom_blur_y_fragment(const gl::rendering_system &rs)
		: Base(rs,
			   gl::device_pipeline_graphics_configurations{},
			   "fullscreen_triangle.vert", "hdr_bloom_blur_y.frag")
	{
		draw_task.attach_pipeline(pipeline);
		pipeline["config_t.dir"] = glm::vec2(0, 1);
	}
