...
extern const std::uint32_t Crc32Lookup[MaxSlice][256]; // extern is needed to keep compiler happy
#endif


/// compute CRC32 (bitwise algorithm)
std::uint32_t crc32_bitwise(const void* data, std::size_t length, std::uint32_t previousCrc32)
{
	std::uint32_t crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
...
std::uint32_t crc32_halfbyte(const void* data, std::size_t length, std::uint32_t previousCrc32)
{
	std::uint32_t crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
	const std::uint8_t* current = (const std::uint8_t*) data;

	/// look-up table for half-byte, same as crc32Lookup[0][16*i]
	static const std::uint32_t Crc32Lookup16[16] =
	{
...
std::uint32_t crc32_1byte(const void* data, std::size_t length, std::uint32_t previousCrc32)
{
	std::uint32_t crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
	const std::uint8_t* current = (const std::uint8_t*) data;
...
std::uint32_t crc32_1byte_tableless(const void* data, std::size_t length, std::uint32_t previousCrc32)
{
	std::uint32_t crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
	const std::uint8_t* current = (const std::uint8_t*) data;
	...
	while (length-- != 0)
	{
		std::uint8_t s = std::uint8_t(crc) ^ *current++;
		...
		std::uint32_t low = (s ^ (s << 6)) & 0xFF;
		std::uint32_t a = (low * ((1 << 23) + (1 << 14) + (1 << 2)));
		crc = (crc >> 8) ^
			(low * ((1 << 24) + (1 << 16) + (1 << 8))) ^
			a ^
			(a >> 1) ^
		...
		// Hagai's code:
		/*std::uint32_t t = (s ^ (s << 6)) << 24;

		// some temporaries to optimize XOR
		std::uint32_t x = (t >> 1) ^ (t >> 2);
		std::uint32_t y = x ^ (x >> 3);
		std::uint32_t z = (t >> 12) ^ (t >> 16);
...
/// compute CRC32 (byte algorithm) without lookup tables
std::uint32_t crc32_1byte_tableless2(const void* data, std::size_t length, std::uint32_t previousCrc32)
{
	...
	while (length-- != 0)
	{
		...
		std::uint32_t c = (((crc << 31) >> 31) & ((Polynomial >> 7) ^ (Polynomial >> 1))) ^
		...
		crc = ((std::uint32_t)crc >> 8) ^ c; // convert to unsigned integer before right shift
	}
...
std::uint32_t crc32_4bytes(const void* data, std::size_t length, std::uint32_t previousCrc32)
{
	std::uint32_t  crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
	const std::uint32_t* current = (const std::uint32_t*) data;

	// process four bytes at once (Slicing-by-4)
	while (length >= 4)
	{
#if __BYTE_ORDER == __BIG_ENDIAN
		std::uint32_t one = *current++ ^ swap(crc);
		crc = Crc32Lookup[0][one & 0xFF] ^
			Crc32Lookup[1][(one >> 8) & 0xFF] ^
			Crc32Lookup[2][(one >> 16) & 0xFF] ^
			Crc32Lookup[3][(one >> 24) & 0xFF];
#else
		std::uint32_t one = *current++ ^ crc;
	...
	const std::uint8_t* currentChar = (const std::uint8_t*) current;
...
std::uint32_t crc32_8bytes(const void* data, std::size_t length, std::uint32_t previousCrc32)
{
	std::uint32_t crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
	const std::uint32_t* current = (const std::uint32_t*) data;
...
std::uint32_t crc32_4x8bytes(const void* data, std::size_t length, std::uint32_t previousCrc32)
{
	std::uint32_t crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
	const std::uint32_t* current = (const std::uint32_t*) data;
	...
	const std::size_t BytesAtOnce = 8 * Unroll;
...
std::uint32_t crc32_16bytes(const void* data, std::size_t length, std::uint32_t previousCrc32)
{
	std::uint32_t crc = ~previousCrc32; // same as previousCrc32 ^ 0xFFFFFFFF
	const std::uint32_t* current = (const std::uint32_t*) data;
	...
	while (length >= BytesAtOnce)
	{
		for (std::size_t unrolling = 0; unrolling < Unroll; unrolling++)
		{
			...
			std::uint32_t one = *current++ ^ swap(crc);
			std::uint32_t two = *current++;
			std::uint32_t three = *current++;
			std::uint32_t four = *current++;
...
std::uint32_t crc32_16bytes_prefetch(const void* data, std::size_t length, std::uint32_t previousCrc32, std::size_t prefetchAhead)
{
...
std::uint32_t crc32_fast(const void* data, std::size_t length, std::uint32_t previousCrc32)
{
#ifdef CRC32_USE_LOOKUP_TABLE_SLICING_BY_16
...
const std::uint32_t Crc32Lookup[MaxSlice][256] =
{
