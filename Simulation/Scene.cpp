...
Object *Scene::create_object(const std::vector<ObjectVertexData> &vertices, const std::vector<unsigned> &ind, Material &&mat) {
	...
	if (diffuse != nullptr) {
		md.diffuse.tex_handler = diffuse->get_texture_handle();
		diffuse->make_resident();
	}
	if (specular != nullptr) {
		md.specular.tex_handler = specular->get_texture_handle();
		specular->make_resident();
	}
	if (heightmap != nullptr) {
		md.heightmap.tex_handler = heightmap->get_texture_handle();
		heightmap->make_resident();
	}
	if (alphamap != nullptr) {
		md.alphamap.tex_handler = alphamap->get_texture_handle();
		alphamap->make_resident();
	}
	if (tex0 != nullptr) {
