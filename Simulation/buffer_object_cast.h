...
BufferTypeTo buffer_object_cast(BufferTypeFrom &&s) {
	...
	static_assert(std::is_base_of<buffer_object<T::T, T::access_usage>, T>::value, "BufferTypeTo must be a buffer object");
	static_assert(std::is_base_of<buffer_object<S::T, S::access_usage>, S>::value, "BufferTypeFrom must be a buffer object");
