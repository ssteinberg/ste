...
class AtomicCounterBufferGeneric : virtual public shader_layout_bindable_resource<atomic_buffer_object_layout_binding_type> {
};

template <BufferUsage::buffer_usage U = BufferUsage::BufferUsageNone>
class AtomicCounterBuffer : public buffer_object_layout_bindable<GLuint, atomic_buffer_object_layout_binding_type, U>, virtual public AtomicCounterBufferGeneric {
private:
...
public:
	...
	void bind(const layout_binding &sampler) const final override { LayoutBinder::bind(id, sampler, GL_ATOMIC_COUNTER_BUFFER); };
	void unbind(const layout_binding &sampler) const final override { LayoutBinder::unbind(sampler, GL_ATOMIC_COUNTER_BUFFER); };
